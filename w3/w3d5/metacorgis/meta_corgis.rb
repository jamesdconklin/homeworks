...
class MetaCorgiPerkPackage
  ...
  #     base = "get_#{name}_"
  #
  #     info = @shopping_list.send("#{base}info".to_sym, @package_id)
  #     title = name.to_s.split('_').map(&:capitalize).join(' ')
  #     happiness = @shopping_list.send("#{base}happiness".to_sym, @package_id)
  #     result = "#{title}: #{info}: #{happiness} licks"
  #     return happiness > 30 ? "* #{result}" : result
  #   end
  ...
  def self.define_perk(name)
    define_method(name) do
      method_base = "get_#{name}_"
      info_sym = (method_base + "info").to_sym
      happiness_sym = (method_base + "happiness").to_sym
      info = @shopping_list.send(info_sym, @package_id)
      happiness = @shopping_list.send(happiness_sym, @package_id)
      title = name.to_s.split('_').map(&:capitalize).join(' ')
      result = "#{title}: #{info}: #{happiness} licks"
      happiness > 30 ? "* #{result}" : result
    end
